!Fourth component of dyadic array: GA(4)
!for points in same layer
double precision function fun_GA4_ii_Re(kr) result(y)
use myfargs
implicit none
!Input
real*8 kr
...
complex*16 kz1, kz2, Ye1, Ye2, Yh1, Yh2, factYe, factYh, Yei, Yhi, kzi
complex*16 ki, ei, c3, c4, c5
complex*16 j, argB
complex*16 segno
integer NZ, IERR
real*8 mu0
...
!Compute Green
fun_Ie=0.5d0*Yei*(exp(-j*kzi*zzam)+segno*factYe*exp(-j*kzi*(segno*zzp)))
fun_Ih=0.5d0*Yhi*(exp(-j*kzi*zzam)+segno*factYh*exp(-j*kzi*(segno*zzp)))
!Constant
c3=ki**2
c4=(kzi**2)/(ki**2)
c5=mu0/ei
fun=(c5/(j*omega))*(fun_Ie-(c3/(krho**2)*(c4*fun_Ie-fun_Ih)))/mu0
