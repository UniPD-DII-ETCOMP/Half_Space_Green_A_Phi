...
C         LARGEST INTEGER, U3=myI1MACH(9). THUS, THE MAGNITUDE OF ZETA
...
C                 PP 265-273.
C
C***ROUTINES CALLED  CACAI,CBKNU,myI1MACH,R1MACH
C***END PROLOGUE  CAIRY
     ...
     * Z3I, Z3R, R1MACH, BB, ALAZ
      INTEGER ID, IERR, IFLAG, K, KODE, K1, K2, MR, NN, NZ, myI1MACH
      DIMENSION CY(1)
...
C         INTEGER, U3=myI1MACH(9). THUS, THE MAGNITUDE OF Z AND FNU+N-1 IS
...
C                 PP 265-273.
C
C***ROUTINES CALLED  CACON,CBKNU,CBUNK,CUOIK,myI1MACH,R1MACH
...
C                 PP 265-273.
C
C***ROUTINES CALLED  CBINU,myI1MACH,R1MACH
C***END PROLOGUE  CBESI
     ...
     * TOL, XX, YY, R1MACH, AZ, FN, BB, ASCLE, RTOL, ATOL
      INTEGER I, IERR, INU, K, KODE, K1, K2, N, NN, NZ, myI1MACH
      DIMENSION CY(N)
      DATA PI /3.14159265358979324E0/
      DATA CONE / (1.0E0,0.0E0) /
C
...
C
     ...
     * TOL, UFL, XX, YY, R1MACH, BB
      INTEGER IERR, K, KODE, K1, K2, MR, N, NN, NUF, NW, NZ, myI1MACH
      DIMENSION CY(N)
...
C                 PP 265-273.
C
C***ROUTINES CALLED  CBESI,CBESK,myI1MACH,R1MACH
...
C
     ...
     * ATOL, TOL, AA, BB, FFNU, HPI, RHPI, R1M5
      INTEGER I, IERR, IFNU, K, KODE, K1, K2, N, NZ, NZ1, NZ2, myI1MACH,
     *I4
...
C***END PROLOGUE  CBIRY
     ...
     * TOL, TTH, ZI, ZR, Z3I, Z3R, R1MACH
      INTEGER ID, IERR, K, KODE, K1, K2, NZ, myI1MACH
      DIMENSION CY(2)
...
C
C***ROUTINES CALLED  CKSCL,CSHCH,GAMLN,myI1MACH,R1MACH,CUCHK
...
C     12.LE.E.LE.60. E IS COMPUTED FROM 2**(-E)=B**(1-myI1MACH(11))=
C     TOL WHERE B IS THE BASE OF THE ARITHMETIC.
C-----------------------------------------------------------------------
      T1 = FLOAT(myI1MACH(11)-1)*R1MACH(5)*3.321928094E0
      T1 = AMAX1(T1,12.0E0)
      T1 = AMIN1(T1,60.0E0)
      T2 = TTH*T1 - 6.0E0
...
c*********************************************************************72
c
cc myD1MACH returns double precision real machine-dependent constants.
c
c  Discussion:
c
c    myD1MACH can be used to obtain machine-dependent parameters
...
c
c    myD1MACH ( 1) = B**(EMIN-1), the smallest positive magnitude.
c    myD1MACH ( 2) = B**EMAX*(1 - B**(-T)), the largest magnitude.
c    myD1MACH ( 3) = B**(-T), the smallest relative spacing.
c    myD1MACH ( 4) = B**(1-T), the largest relative spacing.
c    myD1MACH ( 5) = LOG10(B)
c
c  Licensing:
c
...
c
c    Output, double precision myD1MACH, the value of the constant.
c
      implicit none

      double precision myD1MACH
      integer i

      if ( i < 1 ) then
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'myD1MACH - Fatal error!'
        ...
        myD1MACH = 4.450147717014403D-308
      else if ( i == 2 ) then
        myD1MACH = 8.988465674311579D+307
      else if ( i == 3 ) then
        myD1MACH = 1.110223024625157D-016
      else if ( i == 4 ) then
        myD1MACH = 2.220446049250313D-016
      else if ( i == 5 ) then
        myD1MACH = 0.301029995663981D+000
      else if ( 5 < i ) then
        write ( *, '(a)' ) ' '
        write ( *, '(a)' ) 'myD1MACH - Fatal error!'
        write ( *, '(a)' ) '  The input argument I is out of bounds.'
...
C                 BY D. E. AMOS, SAND83-0083, MAY, 1983.
C***ROUTINES CALLED  myI1MACH,myD1MACH
C***END PROLOGUE  DGAMLN
      DOUBLE PRECISION CF, CON, FLN, FZ, GLN, RLN, S, TLG, TRM, TST,
     * T1, WDTOL, Z, ZDMY, ZINC, ZM, ZMIN, ZP, ZSQ, myD1MACH
      INTEGER I, IERR, I1M, K, MZ, NZ, myI1MACH
...
C***REFERENCES  COMPUTATION OF BESSEL FUNCTIONS OF COMPLEX ARGUMENT
C                 BY D. E. AMOS, SAND83-0083, MAY, 1983.
C***ROUTINES CALLED  myI1MACH,R1MACH
C***END PROLOGUE  GAMLN
C
      INTEGER I, I1M, K, MZ, NZ, IERR, myI1MACH
...
c*********************************************************************72
c
cc myI1MACH returns integer machine dependent constants.
c
c  Discussion:
c
c    Input/output unit numbers.
c
c      myI1MACH(1) = the standard input unit.
c      myI1MACH(2) = the standard output unit.
c      myI1MACH(3) = the standard punch unit.
c      myI1MACH(4) = the standard error message unit.
c
c    Words.
c
c      myI1MACH(5) = the number of bits per integer storage unit.
c      myI1MACH(6) = the number of characters per integer storage unit.
c
c    Integers.
c
...
c    where 0 <= X(1:S-1) < A.
c
c      myI1MACH(7) = A, the base.
c      myI1MACH(8) = S, the number of base A digits.
c      myI1MACH(9) = A**S-1, the largest integer.
c
...
c
c      myI1MACH(10) = B, the base.
c
c    Single precision
c
c      myI1MACH(11) = T, the number of base B digits.
c      myI1MACH(12) = EMIN, the smallest exponent E.
c      myI1MACH(13) = EMAX, the largest exponent E.
c
c    Double precision
c
c      myI1MACH(14) = T, the number of base B digits.
c      myI1MACH(15) = EMIN, the smallest exponent E.
c      myI1MACH(16) = EMAX, the largest exponent E.
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c    1 <= I <= 16.
c
c    Output, integer myI1MACH, the value of the chosen parameter.
c
      implicit none
...
C
C***ROUTINES CALLED  ZASYI,ZBKNU,ZMLRI,ZSERI,ZS1S2,myD1MACH,ZABS
...
C     COMPLEX CSGN,CSPN,C1,C2,Y,Z,ZN,CY
     ...
     * RL, SGN, TOL, YY, YR, YI, ZR, ZI, ZNR, ZNI, myD1MACH, ZABS
      INTEGER INU, IUF, KODE, MR, N, NN, NW, NZ
...
C     HALF Z PLANE
C
C***ROUTINES CALLED  ZBINU,ZBKNU,ZS1S2,myD1MACH,ZABS,ZMLT
...
C    *S1,S2,Y,Z,ZN
     ...
     * YY, ZEROR, ZI, ZNI, ZNR, ZR, myD1MACH, ZABS
      INTEGER I, INU, IUF, KFLAG, KODE, MR, N, NN, NW, NZ
...
C         THE MAGNITUDE OF ZETA=(2/3)*Z**1.5 EXCEEDS U1=SQRT(0.5/UR),
C         THEN LOSSES EXCEEDING HALF PRECISION ARE LIKELY AND AN ERROR
C         FLAG IERR=3 IS TRIGGERED WHERE UR=DMAX1(myD1MACH(4),1.0D-18) IS
...
C                 PP 265-273.
C
C***ROUTINES CALLED  ZACAI,ZBKNU,ZEXP,ZSQRT,ZABS,myI1MACH,myD1MACH
...
C     COMPLEX AI,CONE,CSQ,CY,S1,S2,TRM1,TRM2,Z,ZTA,Z3
     ...
     * ZEROR, ZI, ZR, ZTAI, ZTAR, Z3I, Z3R, myD1MACH, ZABS, ALAZ, BB
      INTEGER ID, IERR, IFLAG, K, KODE, K1, K2, MR, NN, NZ, myI1MACH
...
C-----------------------------------------------------------------------
      AA=0.5D0/TOL
      BB=DBLE(FLOAT(myI1MACH(9)))*0.5D0
      AA=DMIN1(AA,BB)
      AA=AA**TTH
      IF (AZ.GT.AA) GO TO 260
      AA=DSQRT(AA)
...
C     NZ.LT.0 INDICATES AN OVERFLOW ON KODE=1.
C
C***ROUTINES CALLED  myD1MACH,ZABS,ZDIV,ZEXP,ZMLT,ZSQRT
...
C     COMPLEX AK1,CK,CONE,CS1,CS2,CZ,CZERO,DK,EZ,P1,RZ,S2,Y,Z
     ...
     * S2R, TOL, TZI, TZR, YI, YR, ZEROI, ZEROR, ZI, ZR, myD1MACH, ZABS
      INTEGER I, IB, IL, INU, J, JL, K, KODE, KODED, M, N, NN, NZ
      DIMENSION YR(N), YI(N)
...
C         CONSEQUENTLY, IF EITHER ONE EXCEEDS U1=SQRT(0.5/UR), THEN
C         LOSSES EXCEEDING HALF PRECISION ARE LIKELY AND AN ERROR FLAG
C         IERR=3 IS TRIGGERED WHERE UR=DMAX1(myD1MACH(4),1.0D-18) IS
C         DOUBLE PRECISION UNIT ROUNDOFF LIMITED TO 18 DIGITS PRECISION.
C         IF EITHER IS LARGER THAN U2=0.5/UR, THEN ALL SIGNIFICANCE IS
C         LOST AND IERR=4. IN ORDER TO USE THE INT FUNCTION, ARGUMENTS
C         MUST BE FURTHER RESTRICTED NOT TO EXCEED THE LARGEST MACHINE
...
C                 PP 265-273.
C
C***ROUTINES CALLED  ZACON,ZBKNU,ZBUNK,ZUOIK,ZABS,myI1MACH,myD1MACH
...
C     COMPLEX CY,Z,ZN,ZT,CSGN
     ...
     * ZNI, ZNR, ZR, ZTI, myD1MACH, ZABS, BB, ASCLE, RTOL, ATOL, STI,
     * CSGNR, CSGNI
      INTEGER I, IERR, INU, INUH, IR, K, KODE, K1, K2, M,
     * MM, MR, N, NN, NUF, NW, NZ, myI1MACH
...
C                 PP 265-273.
C
C***ROUTINES CALLED  ZBINU,ZABS,myI1MACH,myD1MACH
...
C     COMPLEX CONE,CSGN,CW,CY,CZERO,Z,ZN
     ...
     * ZR, myD1MACH, AZ, BB, FN, ZABS, ASCLE, RTOL, ATOL, STI
      INTEGER I, IERR, INU, K, KODE, K1,K2,N,NZ,NN, myI1MACH
      DIMENSION CYR(N), CYI(N)
      DATA PI /3.14159265358979324D0/
...
C     COMPLEX CI,CSGN,CY,Z,ZN
     ...
     * myD1MACH, BB, FN, AZ, ZABS, ASCLE, RTOL, ATOL, STI
      INTEGER I, IERR, INU, INUH, IR, K, KODE, K1, K2, N, NL, NZ, myI1MACH
      DIMENSION CYR(N), CYI(N)
      DATA HPI /1.57079632679489662D0/
C
...
C     COMPLEX CY,Z
     ...
     * FNU, FNUL, RL, R1M5, TOL, UFL, ZI, ZR, myD1MACH, ZABS, BB
      INTEGER IERR, K, KODE, K1, K2, MR, N, NN, NUF, NW, NZ, myI1MACH
      DIMENSION CYR(N), CYI(N)
...
C                 PP 265-273.
C
C***ROUTINES CALLED  ZBINU,ZABS,ZDIV,ZSQRT,myD1MACH,myI1MACH
...
C     COMPLEX BI,CONE,CSQ,CY,S1,S2,TRM1,TRM2,Z,ZTA,Z3
     ...
     * TRM2R, TTH, ZI, ZR, ZTAI, ZTAR, Z3I, Z3R, myD1MACH, ZABS
      INTEGER ID, IERR, K, KODE, K1, K2, NZ, myI1MACH
...
C
C***ROUTINES CALLED  DGAMLN,myI1MACH,myD1MACH,ZKSCL,ZSHCH,ZUCHK,ZABS,ZDIV,
...
C     COMPUTE R2=F(E). IF CABS(Z).GE.R2, USE FORWARD RECURRENCE TO
C     DETERMINE THE BACKWARD INDEX K. R2=F(E) IS A STRAIGHT LINE ON
C     12.LE.E.LE.60. E IS COMPUTED FROM 2**(-E)=B**(1-myI1MACH(14))=
C     TOL WHERE B IS THE BASE OF THE ARITHMETIC.
C-----------------------------------------------------------------------
      T1 = DBLE(FLOAT(myI1MACH(14)-1))
      T1 = T1*myD1MACH(5)*3.321928094D0
      T1 = DMAX1(T1,12.0D0)
      T1 = DMIN1(T1,60.0D0)
      T2 = TTH*T1 - 6.0D0
...
C     ON IFORM=1 AND THE EXPANSION FOR J(FNU,Z) ON IFORM=2
C
C***ROUTINES CALLED  ZUNI1,ZUNI2,ZABS,myD1MACH
...
C     MILLER ALGORITHM NORMALIZED BY A NEUMANN SERIES.
C
C***ROUTINES CALLED  DGAMLN,myD1MACH,ZABS,ZEXP,ZLOG,ZMLT
...
C     COMPUTATION MUST BE COMPLETED IN ANOTHER ROUTINE WITH N=N-ABS(NZ).
C
C***ROUTINES CALLED  DGAMLN,myD1MACH,ZUCHK,ZABS,ZDIV,ZLOG,ZMLT
...
C-----------------------------------------------------------------------
C     EXP(-ALIM)=EXP(-ELIM)/TOL=APPROX. ONE PRECISION ABOVE THE
C     UNDERFLOW LIMIT = ASCLE = myD1MACH(1)*SS*1.0D+3
C-----------------------------------------------------------------------
      S1R = WR(1)
      S1I = WI(1)
      S2R = WR(2)
...
C
C      Y ENTERS AS A SCALED QUANTITY WHOSE MAGNITUDE IS GREATER THAN
C      EXP(-ALIM)=ASCLE=1.0E+3*myD1MACH(1)/TOL. THE TEST IS MADE TO SEE
...
C         1 COMPUTES ALL EXCEPT ASUM AND BSUM.
C
C***ROUTINES CALLED  ZABS,ZDIV,ZLOG,ZSQRT,myD1MACH
...
C    *ZETA2,ZTH
     ...
     * ZETA2R, ZI, ZR, ZTHI, ZTHR, ZABS, AC, myD1MACH
      INTEGER IAS, IBS, IPMTR, IS, J, JR, JU, K, KMAX, KP1, KS, L, LR,
     * LRP1, L1, L2, M, IDUM
      DIMENSION AR(14), BR(14), C(105), ALFA(180), BETA(210), GAMA(30),
...
C     Y(I)=CZERO FOR I=NLAST+1,N
C
C***ROUTINES CALLED  ZUCHK,ZUNIK,ZUOIK,myD1MACH,ZABS
...
C    *S2,Y,Z,ZETA1,ZETA2
     ...
     * ZETA1R, ZETA2I, ZETA2R, ZI, ZR, CYR, CYI, myD1MACH, ZABS
      INTEGER I, IFLAG, INIT, K, KODE, M, N, ND, NLAST, NN, NUF, NW, NZ
...
C     Y(I)=CZERO FOR I=NLAST+1,N
C
C***ROUTINES CALLED  ZAIRY,ZUCHK,ZUNHJ,ZUOIK,myD1MACH,ZABS
...
C        ZETA1,ZETA2.
C
C***ROUTINES CALLED  ZDIV,ZLOG,ZSQRT,myD1MACH
...
C    *ZETA2,ZN,ZR
     ...
     * ZETA2I, ZETA2R, ZNI, ZNR, ZRI, ZRR, myD1MACH
...
C     NZ=-1 MEANS AN OVERFLOW WILL OCCUR
C
C***ROUTINES CALLED  ZKSCL,ZS1S2,ZUCHK,ZUNIK,myD1MACH,ZABS
...
C    *C1,C2,PHI,PHID,RZ,SUM,SUMD,S1,S2,Y,Z,ZETA1,ZETA1D,ZETA2,ZETA2D,ZR
     ...
     * ZET1DI, ZET1DR, ZET2DI, ZET2DR, ZI, ZR, ZRI, ZRR, myD1MACH, ZABS
      INTEGER I, IB, IFLAG, IFN, IL, INIT, INU, IUF, K, KDFLG, KFLAG,
...
C     NZ=-1 MEANS AN OVERFLOW WILL OCCUR
C
C***ROUTINES CALLED  ZAIRY,ZKSCL,ZS1S2,ZUCHK,ZUNHJ,myD1MACH,ZABS
...
C    *S1,S2,Y,Z,ZB,ZETA1,ZETA1D,ZETA2,ZETA2D,ZN,ZR
     ...
     * ZET2DR, ZI, ZNI, ZNR, ZR, ZRI, ZRR, myD1MACH, ZABS
      INTEGER I, IB, IFLAG, IFN, IL, IN, INU, IUF, K, KDFLG, KFLAG, KK,
     * KODE, MR, N, NAI, NDAI, NW, NZ, IDUM, J, IPARD, IC
...
C             ANOTHER ROUTINE
C
C***ROUTINES CALLED  ZUCHK,ZUNHJ,ZUNIK,myD1MACH,ZABS,ZLOG
...
C    *ZR
     ...
     * ZNI, ZNR, ZR, ZRI, ZRR, myD1MACH, ZABS
...
C
C***ROUTINES CALLED  myD1MACH,ZBKNU,ZRATI,ZABS
...
C     COMPLEX CINU,CSCL,CT,CW,C1,C2,RCT,ST,Y,ZR
     ...
     * STI, STR, TOL, YI, YR, ZRI, ZRR, ZABS, myD1MACH
      INTEGER I, KODE, N, NW, NZ
